...
return nil
end,sdkPayload=function(n)local e={name=o["NAME"],version=o["VERSION"]}if n then
e.testing_mode=n
end
return e
...
return e
end,payloadAsJsonString=function(e,t)return s.encode(n.payload(e,t))end,printEnvironmentInformation=function(r,i,l,u)e.debug("==============================================")e.debug(n.payloadAsJsonString(i,l))local c=nil
local d=nil
local s=nil
if t.getSessionStarted()then
c=system.getInfo("iosAdvertisingIdentifier")d=system.getInfo("iosIdentifierForVendor")s=system.getInfo("iosAdvertisingTrackingEnabled")end
local l=nil
local n=nil
if r~=nil then
...
e.release("IP Address: "..tostring(t.getIpAddress()))e.release("Device name: "..system.getInfo("name"))e.release("Model name: "..system.getInfo("model"))e.release("Device ID: "..system.getInfo("deviceID"))e.release('IDFA (iOS only): '..tostring(c))e.release('IDFV (iOS only): '..tostring(d))e.release('Limit ad tracking (iOS only): '..tostring(s))e.release("Environment: "..system.getInfo("environment"))e.release("Platform name: "..system.getInfo("platformName"))e.release("Platform version: "..system.getInfo("platformVersion"))e.release("Corona version: "..system.getInfo("version"))e.release("Corona build: "..system.getInfo("build"))e.release("Architecture: "..system.getInfo("architectureInfo"))e.release("Locale-Country: "..system.getPreference("locale","country"))e.release("Locale-Language: "..system.getPreference("locale","language"))e.release("Timeout: "..tostring(u).."s")e.release("Corona Simulator: "..tostring(t.isSimulator()))e.release("iOS Simulator: "..tostring(t.isXcodeSimulator()))if i~=nil then e.release("Installed in this device: "..tostring(a.isInstallRegistered(i)))end
...
end)package.preload['revmob_client']=(function(...)local m=require('json')local g=require('socket.http')local l=require("ltn12")local e=require('revmob_about')local n=require('revmob_log')local t=require('revmob_messages')local o=require('revmob_events')local u=require('revmob_context')local p=require('revmob_cache')local i=require('revmob_device')local d=require('revmob_utils')REVMOB_ID_IOS='iPhone OS'REVMOB_ID_ANDROID='Android'local e
local v=30
local h='https://ios.revmob.com'local c='https://android.revmob.com'local L="app"local D=false
local k=true
...
local function h(t,i,o,n)if n==nil then
...
c(I(e.appId),n)end
local function a(l,a,i,r)local i=b(i)if e.sessionStarted then
if i~=nil then
n.info("Ad registered with Placement ID "..i)end
c(h(e.appId,l,a,i),r)else
...
local u
local function h(n)local t={}local e=""local i,e,t=g.request{method="POST",url=n,source=l.source.string(e),headers={["Content-Length"]=tostring(#e),["Content-Type"]="application/json"},sink=l.sink.table(t),}return u(n,e,t)end
...
e={TEST_WITH_ADS="with_ads",TEST_WITHOUT_ADS="without_ads",TEST_DISABLED=nil,appId=nil,appIds=nil,sessionStarted=false,testMode=nil,listenersRegistered=false,serverEndPoints={},ipAddress=nil,startSession=function(l,r)local t=b(l)if w(t)then
if not e.sessionStarted then
e.appId=t
e.appIds=l
local e=function(l)local a=l.status or l.statusCode
if(a==200)then
local a,l=pcall(m.decode,l.response)if(a~=nil and l~=nil)then
local a=l['links']for t,n in ipairs(a)do
e.serverEndPoints[n.rel]=n.href
end
e.ipAddress=l['ip_address']e.sessionStarted=true
if r~=nil then r({type=o.SESSION_IS_STARTED,ad="session"})end
...
i.setSessionStarted(e.sessionStarted)if r~=nil then r({type=o.SESSION_NOT_STARTED,ad="session"})end
...
local i,e=pcall(m.decode,l.response)if(not i or e==nil)then
...
end,openWebView=function(n)r=display.newGroup()d.createImageFromBase64(d.CLOSE_BUTTON,d.CLOSE_BUTTON_IMAGE)local e=display.newImage(d.CLOSE_BUTTON_IMAGE,system.TemporaryDirectory)e.x,e.y=display.contentWidth-e.width/2,e.height/2+display.screenOriginY
e:addEventListener("touch",y)r:insert(e)local e=native.newWebView(0,e.height+display.screenOriginY,display.contentWidth,display.contentHeight-display.screenOriginY*2)e:request(n)r:insert(e)end,getLink=function(n,e)for t,e in ipairs(e)do
if e.rel==n then
return e.href
end
end
...
e=""end
local i,e,t=g.request{method="POST",url=n,source=l.source.string(e),headers={["Content-Length"]=tostring(#e),["Content-Type"]="application/json"},sink=l.sink.table(t),}return u(n,e,t)end,getMarketURLWithoutFollow=function(e)f=false
local e=h(e)f=true
return e
...
end)package.preload['revmob_fullscreen_web']=(function(...)local l=require('revmob_log')local a=require('revmob_messages')local r=require('revmob_events')local n=require('revmob_client')require('revmob_sound_player')local i="fullscreen"local o
...
local t=o['fullscreen']['links']e.impressionUrl=n.getLink('impressions',t)e.clickUrl=n.getLink('clicks',t)e.htmlUrl=n.getLink('html',t)l.release("Fullscreen loaded - "..e.campaignId)if e.listener~=nil then e.listener({type=r.AD_RECEIVED,ad=i})end
if e.autoshow then
e:show()end
end
end
...
if o.errorCode then
l.release("Error: "..tostring(o.errorMessage))end
return true
end
...
return r
end)package.preload['revmob_fullscreen']=(function(...)local o=require('revmob_log')local n=require('revmob_client')local c=require('revmob_fullscreen_static')local a=require('revmob_fullscreen_web')local l=require("socket.url")local t="fullscreen"RevMobFullscreen={params=nil,view=nil,listener=nil,placementIds=nil,autoshow=true,new=function(n)local e=n or{}setmetatable(e,RevMobFullscreen)e.params=n
return e
end,load=function(e)local l=function(i)local i,t=n.theFetchSucceed(t,i,e.listener)if i then
...
e.clickSoundURL=i
local t=t['fullscreen']['links']local r=n.getLink('impressions',t)local i=n.getLink('clicks',t)local d=n.getLink('html',t)local u=n.getLink('image',t)local f=n.getLink('close_button',t)local s=n.getLink('dsp_url',t)local t=n.getLink('dsp_html',t)local n=""local l=l.parse(i)local l=l["query"]local l=l:split("&")for e=1,#l do
if string.find(l[e],"campaign_id=")~=nil then
local e=l[e]:split("=")n=e[2]end
end
if t~=nil then
o.debug("DSP html fullscreen")e.createFile(t)e.view=a.new(e.params)e.view.dspHtml=t
...
e.view:show()end
elseif s~=nil then
o.debug("DSP url fullscreen")e.view=a.new(e.params)e.view.htmlUrl=s
e.view.impressionUrl=r
e.view.clickUrl=i
e.view.campaignId=n
e.view.autoshow=e.autoshow
...
e.view:show()end
elseif d~=nil then
o.debug("Rich fullscreen")e.view=a.new(e.params)e.view.htmlUrl=d
e.view.impressionUrl=r
e.view.clickUrl=i
e.view.campaignId=n
e.view.autoshow=e.autoshow
...
t={}end
local e=1
local i,r=string.find(self,o,e)while i do
table.insert(t,string.sub(self,e,i-1))e=r+1
i,r=string.find(self,o,e)end
table.insert(t,string.sub(self,e))return t
end
n.fetchFullscreen(e.placementIds,l)end,hide=function(e)e.autoshow=false
...
end)package.preload['revmob_banner_web']=(function(...)local o=require('revmob_log')local a=require('revmob_messages')local l=require('revmob_events')local t=require('revmob_utils')local s=require('revmob_device')local n=require('revmob_client')require('revmob_sound_player')local r="banner"local i
...
end,load=function(e,i)e.networkListener=function(t)local i,t=n.theFetchSucceed(r,t,e.listener)if i then
local t=t['banners'][1]['links']e.impressionUrl=n.getLink('impressions',t)e.clickUrl=n.getLink('clicks',t)e.htmlUrl=n.getLink('html',t)o.release("Banner loaded - "..e.campaignId)if e.listener~=nil then e.listener({type=l.AD_RECEIVED,ad=r})end
...
end,update=function(e,n,i,t,r,o)e:setPosition(n,i)e:setDimension(t,r,o)end,release=function(e)if e.webView then
...
return i
end)package.preload['revmob_banner_static']=(function(...)local i=require('revmob_log')local s=require('revmob_messages')local l=require('revmob_events')local t=require('revmob_utils')local a=require('revmob_device')local n=require('revmob_client')require('revmob_sound_player')local r="banner"local o
...
e.listener({type=l.AD_CLICKED,ad=r})end
local t=nil
if e.followRedirect==true then
t=n.getMarketURL(e.clickUrl)else
t=n.getMarketURLWithoutFollow(e.clickUrl)end
...
e.image.touch=function(n)return true end
e.image:addEventListener("tap",e.image)e.image:addEventListener("touch",e.image)e.component:insert(1,e.image)i.release("Banner loaded - "..e.campaignId)if e.listener~=nil then e.listener({type=l.AD_RECEIVED,ad=r})end
if e.autoshow then
e:show()end
end
...
return o
end)package.preload['revmob_banner']=(function(...)local o=require('revmob_log')local n=require('revmob_client')local f=require('revmob_banner_static')local a=require('revmob_banner_web')local h=require('revmob_events')local u=require("socket.url")local i="banner"RevMobBanner={params=nil,view=nil,placementIds=nil,listener=nil,autoshow=true,width=nil,height=nil,x=nil,y=nil,rotation=0,new=function(n)local e=n or{}setmetatable(e,RevMobBanner)e.params=n
return e
...
e.clickSoundURL=i
local t=t['banners'][1]['links']local r=n.getLink('impressions',t)local i=n.getLink('clicks',t)local c=n.getLink('image',t)local d=n.getLink('html',t)local s=n.getLink('dsp_url',t)local l=n.getLink('dsp_html',t)local n=""local t=u.parse(i)local t=t["query"]local t=t:split("&")for e=1,#t do
if string.find(t[e],"campaign_id=")~=nil then
local e=t[e]:split("=")n=e[2]end
end
if l~=nil then
o.debug("DSP hmtl banner")if e.params==nil then e.params={}end
e.params["listener"]=RevMobBannerViewListener
e.createFile(l)e.view=a.new(e.params)e.view.dspHtml=l
e.view.impressionUrl=r
e.view.clickUrl=i
e.view.campaignId=n
e.view.autoshow=e.autoshow
...
e.params["listener"]=RevMobBannerViewListener
o.debug("DSP url banner")e.view=a.new(e.params)e.view.htmlUrl=s
e.view.impressionUrl=r
e.view.clickUrl=i
e.view.campaignId=n
e.view.autoshow=e.autoshow
...
if e.params==nil then e.params={}end
e.params["listener"]=RevMobBannerViewListener
o.debug("Rich banner")e.view=a.new(e.params)e.view.htmlUrl=d
e.view.impressionUrl=r
e.view.clickUrl=i
e.view.campaignId=n
e.view.autoshow=e.autoshow
...
e.params["listener"]=RevMobBannerViewListener
e.view=f.new(e.params)e.view.imageUrl=c
e.view.impressionUrl=r
e.view.clickUrl=i
e.view.campaignId=n
e.view.autoshow=e.autoshow
...
RevMobBannerViewListener=function(e)if e.type==h.AD_CLICKED and e.ad=="banner"then
end
end
...
e.followRedirect=l
local t=t['anchor']['links']e.impressionUrl=n.getLink('impressions',t)e.clickUrl=n.getLink('clicks',t)e.campaignId=""local n=s.parse(e.clickUrl)local n=n["query"]local n=n:split("&")for t=1,#n do
if string.find(n[t],"campaign_id=")~=nil then
local n=n[t]:split("=")e.campaignId=n[2]end
end
r.release("Link loaded - "..e.campaignId)if e.publisherListener then e.publisherListener({type=o.AD_RECEIVED,ad=i})end
if e.autoopen then e:open()end
else
if e.publisherListener then e.publisherListener({type=o.AD_NOT_RECEIVED,ad=i})end
end
end
function string:split(r,t)if not t then
t={}end
local e=1
local i,o=string.find(self,r,e)while i do
table.insert(t,string.sub(self,e,i-1))e=o+1
i,o=string.find(self,r,e)end
table.insert(t,string.sub(self,e))return t
end
...
end)package.preload['revmob_popup']=(function(...)local r=require('revmob_log')local a=require('revmob_messages')local o=require('revmob_events')local t=require('revmob_client')require('revmob_sound_player')local d=require("socket.url")local i="popup"local s=2
...
e.clickSoundURL=l
local l=n['pop_up']['links']e.impressionUrl=t.getLink('impressions',l)e.clickUrl=t.getLink('clicks',l)local t=t.getDefaultValue("popup_message")if(n["pop_up"]["message"]~=nil)then
r.release("entrou no popup...")t=n["pop_up"]["message"]end
e.message=t
e.campaignId=""local n=d.parse(e.clickUrl)local n=n["query"]local n=n:split("&")for t=1,#n do
if string.find(n[t],"campaign_id=")~=nil then
local n=n[t]:split("=")e.campaignId=n[2]end
end
r.release("Popup loaded - "..e.campaignId)if e.autoshow then e:show()end
...
e={}end
local n=1
local i,r=string.find(self,o,n)while i do
table.insert(e,string.sub(self,n,i-1))n=r+1
i,r=string.find(self,o,n)end
table.insert(e,string.sub(self,n))return e
end
...
return n end)package.preload['revmob_sound_player']=(function(...)local o=require("audio")local i=require("revmob_download_manager")local e=require('revmob_log')SoundPlayer={}SoundPlayer={playSoundFromURL=function(n)if(n~=nil and string.len(n)>0)then
local r="/"local t=nil
local e=string.find(n,r,1)t=e
while e do
e=string.find(n,r,e+1)if e then
t=e
end
end
local e=string.sub(n,t+1)i.download(n,e)local e=o.play(o.loadSound(i.fileName,i.params.response.baseDirectory))end
