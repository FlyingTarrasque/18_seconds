...
local lvlSelectBtn = display.newText("Level Select", halfW, halfH + 140, fontType, 40)
...
function lvlSelectBtn:tap(event)
	if lvlSelectBtn.taped then
		return true
	end
	lvlSelectBtn.taped = true
	local callback = function()
		storyboard.removeAll()
		storyboard.gotoScene(scenesDir .. "levelSelect", "fade", 500)
	end

	blinkText(lvlSelectBtn, callback)
	return true
end
...
function scene:destroyScene( event )
	...
	lvlSelectBtn:removeEventListener('tap', lvlSelectBtn)
